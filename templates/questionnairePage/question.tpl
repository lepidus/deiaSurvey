<div class="question">
    ...
    {if $question['type'] == $questionTypeConsts['TYPE_SMALL_TEXT_FIELD']}
        <input type="text" id="deiaResponses" class="questionSmallText" name="{$questionId}" value="" required aria-required="true">
    {elseif $question['type'] == $questionTypeConsts['TYPE_TEXT_FIELD']}
        <input type="text" id="deiaResponses" class="questionText" name="{$questionId}" value="" required aria-required="true">
    {elseif $question['type'] == $questionTypeConsts['TYPE_TEXTAREA']}
        <textarea id="deiaResponses" class="questionTextArea" name="{$questionId}" value="" required aria-required="true"></textarea>
        ...
        {foreach from=$question['responseOptions'] item="responseOption"}
            <div id="responseOption-{$responseOption->getId()}">
                <label class="questionCheckbox">
                    <input type="checkbox" id="deiaResponses" name="{$questionId}[]" value="{$responseOption->getId()}">
        ...
        {foreach from=$question['responseOptions'] item="responseOption"}
            <div id="responseOption-{$responseOption->getId()}">
                <label class="questionRadio">
                    <input type="radio" id="deiaResponses" name="{$questionId}[]" value="{$responseOption->getId()}">
                    {$responseOption->getLocalizedOptionText()}
                </label>
                {if $responseOption->hasInputField()}
                    {assign var="optionInputName" value="responseOptionInput-{$responseOption->getId()}"}
    ...
    {elseif $question['type'] == $questionTypeConsts['TYPE_DROP_DOWN_BOX']}
        <select id="deiaResponses" class="questionSelect" name="{$questionId}">
