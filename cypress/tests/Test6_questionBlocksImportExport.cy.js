describe('DEIA Survey - Question blocks import and export', function () {
	const downloadedQuestionBlockPath = 'cypress/downloads/question-blocks.json';
	let questionBlock;
	...
	it('Exports the selected question block', function () {
		...
		cy.get(`tr:contains("${questionBlock.title}")`).invoke('attr', 'id').then((rowId) => {
			...
			return cy.get('input[name="csrfToken"]').first().invoke('val').then((csrfToken) => (
				cy.request({
					...
					expect(body.plugin).to.equal('deiaSurvey');
					expect(body.blocks).to.have.length(1);
					expect(body.blocks[0].title.en_US).to.equal(questionBlock.title);
					expect(body.blocks[0].description.en_US).to.equal(questionBlock.description);
					expect(body.blocks[0].questions[0].questionType).to.equal('TYPE_CHECKBOXES');
					expect(body.blocks[0].questions[0].responseOptions).to.have.length(2);
					return cy.writeFile(downloadedQuestionBlockPath, body);
				})
			));
		});
	...
	it('Imports a question block file', function () {
		...
		cy.readFile(downloadedQuestionBlockPath).then(fileContent => {
			fileContent.blocks[0].title.en_US = "Imported funding DEIA questions";
			const json = JSON.stringify(fileContent);

			cy.wait(1000);
			cy.get('form#deiaQuestionBlocksImportForm input[type=file]').upload(
				{fileContent: json, fileName: 'question-blocks.json', mimeType: 'application/json', encoding: 'utf8'}
			);
		});
